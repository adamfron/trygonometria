...
function draw(svg,fn,color,a,b,isTan=false){svg.innerHTML=`<rect width='${W}' height='${H}' fill='#fff'/>${axes(isTan)}${isTan?`<line x1='${pxX(90)}' y1='${t}' x2='${pxX(90)}' y2='${H-b}' stroke='#fda4af' stroke-dasharray='4 3'/><line x1='${pxX(270)}' y1='${t}' x2='${pxX(270)}' y2='${H-b}' stroke='#fda4af' stroke-dasharray='4 3'/>`:''}<path d='${path(fn,isTan)}' fill='none' stroke='${color}' stroke-width='2.1'/>${markers(a,b,fn,color,isTan)}`;}

export function renderGraphs(sinSvg,cosSvg,tanSvg,alpha,beta){draw(sinSvg,Math.sin,'#2563eb',alpha,beta,false);draw(cosSvg,Math.cos,'#059669',alpha,beta,false);draw(tanSvg,Math.tan,'#dc2626',alpha,beta,true);}
