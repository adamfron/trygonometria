...
export function checkAll(){
 ...
 const result=rows(state.task).map(r=>({k:r.k,ok:checkAnswer(state.task,r.k,document.getElementById(`${r.k}-rn`)?.value,document.getElementById(`${r.k}-rd`)?.value,document.getElementById(`${r.k}-sn`)?.value,document.getElementById(`${r.k}-sd`)?.value,document.getElementById(`${r.k}-s1`)?.value)}));
