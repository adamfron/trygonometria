...
export function renderAnswers(el){
 ...
 if(state.mode!=='pyth'||state.pythSolved) html+=rows(t).map(r=>`<div class='answer-row' data-row='${r.k}'><b>${labels[r.k]}</b> <span class='fraction'><input id='${r.k}-rn'><span class='bar'></span><input id='${r.k}-rd'></span> ${r.s.changed?`=<span class='fraction'><input id='${r.k}-sn'><span class='bar'></span><input id='${r.k}-sd'></span>`:''}</div>`).join('');
 el.innerHTML=html;
}

export function wireFocus(root){root.querySelectorAll('input').forEach(i=>{i.onfocus=()=>state.focusedInput=i; i.onblur=()=>{};});const first=root.querySelector('#missingSide')||root.querySelector('input'); if(first){first.focus(); state.focusedInput=first;}}
